...
async function processEmailBatch() {
    ...
    const emails = await query(`UPDATE email_queue SET status = 'sending', locked_by = $1, locked_at = now()
     WHERE id IN (
     ...
     RETURNING id, user_id, email, subject, body_html, priority, attempts, max_attempts`, [WORKER_ID, BATCH_SIZE]);
    if (emails.length === 0)
        return;
    await Promise.allSettled(emails.map(async (mail) => {
        try {
            await sendEmail({
                to: mail.email,
            ...
            await query(`UPDATE email_queue SET status = 'sent', sent_at = now(), locked_by = NULL
           WHERE id = $1`, [mail.id]);
        }
        catch (err) {
            const attempts = mail.attempts + 1;
            const newStatus = attempts >= mail.max_attempts ? 'failed' : 'pending';
            await query(`UPDATE email_queue SET
             status = $1, attempts = $2, error = $3,
             locked_by = NULL, locked_at = NULL
           WHERE id = $4`, [newStatus, attempts, err.message, mail.id]);
        }
    }));
}
/** 清理卡死的 sending 狀態（超過 5 分鐘未完成）*/
async function releaseStaleLocks() {
    await query(`UPDATE email_queue SET status = 'pending', locked_by = NULL, locked_at = NULL
...
export async function startEmailWorker() {
    ...
    const loop = async () => {
        ...
        try {
            ...
            console.error('[EmailWorker] 批次處理失敗：', err.message);
        }
