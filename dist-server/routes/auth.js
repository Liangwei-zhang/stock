...
import { generateCode, saveEmailCode, verifyEmailCode, upsertUser, createSession, revokeSession, isCodeRateLimited, } from '../services/userService.js';
 ...
 */
router.post('/send-code', rateLimiter(10, 60), // 同 IP 每分鐘最多 10 次
validate(sendCodeSchema), asyncHandler(async (req, res) => {
 ...
 * 驗證碼登入（自動創建新用戶）
 */
router.post('/verify', rateLimiter(20, 60), validate(verifySchema), asyncHandler(async (req, res) => {
    const { email, code, locale, timezone } = req.body;
