...
async function getMaxPortfolio(userId) {
    const row = await queryOne(`SELECT plan FROM users WHERE id = $1`, [userId]);
    const limits = { free: 5, pro: 20, premium: 100 };
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const rows = await query(`SELECT p.id, p.symbol, p.shares, p.avg_cost, p.total_capital,
     ...
     WHERE p.user_id = $1
     ORDER BY p.total_capital DESC`, [userId]);
    res.json(rows);
}));
/** POST /api/portfolio */
router.post('/', validate(addSchema), asyncHandler(async (req, res) => {
    ...
    const [countRow, max] = await Promise.all([
        queryOne(`SELECT COUNT(*) as count FROM user_portfolio WHERE user_id = $1`, [userId]),
    ...
    await query(`INSERT INTO symbols (symbol, name) VALUES ($1, $1)
     ON CONFLICT (symbol) DO NOTHING`, [body.symbol]);
    const totalCapital = parseFloat((body.shares * body.avg_cost).toFixed(2));
    const row = await queryOne(`INSERT INTO user_portfolio
...
router.put('/:id', validate(updateSchema), asyncHandler(async (req, res) => {
    ...
    const existing = await queryOne(`SELECT shares, avg_cost FROM user_portfolio WHERE id = $1 AND user_id = $2`, [id, userId]);
    if (!existing)
    ...
    const updates = ['updated_at = now()', `total_capital = $3`];
    const params = [id, userId, totalCapital];
    const fields = [
    ...
    const row = await queryOne(`UPDATE user_portfolio SET ${updates.join(', ')}
...
router.delete('/:id', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
    const row = await queryOne(`DELETE FROM user_portfolio WHERE id = $1 AND user_id = $2 RETURNING symbol`, [id, userId]);
    if (!row)
        return res.status(404).json({ error: 'Position not found' });
    await delCache('active_symbols');
