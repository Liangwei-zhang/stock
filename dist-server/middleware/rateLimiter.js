 ...
 *   EXPIRE 設置 key 過期
 */
export function rateLimiter(limit, windowSec = 60) {
    return async (req, res, next) => {
        ...
        try {
            ...
            console.warn('[限流] Redis 錯誤，降級放行：', err.message);
        }
        next();
    };
}
...
/** 黑名單中間件 */
export async function blacklistMiddleware(req, res, next) {
    const ip = (req.ip ?? req.socket.remoteAddress ?? 'unknown').replace(/^::ffff:/, '');
...
export async function addToBlacklist(ip, reason = 'manual', ttlSec = 86_400 * 30 // 預設 30 天
) {
    try {
        await redis.setex(`blacklist:${ip}`, ttlSec, reason);
    }
    catch (err) {
        console.error('[黑名單] 添加失敗：', err.message);
    }
}
...
export async function removeFromBlacklist(ip) {
    try {
        ...
        console.error('[黑名單] 移除失敗：', err.message);
    }
}
