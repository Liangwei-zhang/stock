 ...
 * 3. 批量入隊 email_queue
 */
export async function processBuySignal(signal) {
    // 1. 插入 signals 記錄
    const sigRow = await queryOne(`INSERT INTO signals (symbol, type, score, price, reasons, analysis)
    ...
    const subscribers = await query(`SELECT
     ...
     WHERE uw.symbol = $1
       ...
       AND ua.total_capital > 0`, [signal.symbol, signal.score]);
    ...
    for (const sub of subscribers) {
        ...
        // 計算可用現金（總資金 - 持倉總值）
        const portfolioRows = await query(`SELECT COALESCE(SUM(total_capital), 0)::text as total_capital
       FROM user_portfolio WHERE user_id = $1`, [sub.user_id]);
...
//# sourceMappingURL=buyScanner.js.map