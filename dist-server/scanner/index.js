...
async function yahooChartRequest(symbol, params) {
...
async function fetchPrice(symbol) {
    ...
    try {
        ...
        console.warn(`[Scanner] fetchPrice(${symbol}) 失敗：`, err.message);
    }
    return null;
}
/**
...
async function fetchOHLCV(symbol) {
    ...
    if (cached)
        return cached;
    try {
        const rows = await query(`SELECT ts, o, h, l, c, v FROM ohlcv
       WHERE symbol = $1 AND tf = '1d'
       ORDER BY ts DESC LIMIT 120`, [symbol]);
        if (rows.length >= 60) {
            const bars = rows
    ...
    try {
        ...
        if (bars.length > 0) {
            const vals = bars
                ...
                .join(',');
            await pool.query(`INSERT INTO ohlcv(symbol,tf,ts,o,h,l,c,v) VALUES ${vals} ON CONFLICT (symbol,tf,ts) DO NOTHING`).catch(() => { });
            setCache(ck, bars, 300);
            return bars;
        }
    }
    catch (err) {
        console.warn(`[Scanner] fetchOHLCV(${symbol}) Yahoo 失敗：`, err.message);
    }
    return [];
}
// ─────────────────────────────────────────────────────
...
//  SMC 評分（精簡版）
// ─────────────────────────────────────────────────────
async function analyzeSymbol(symbol, _price) {
    ...
    const reasons = [];
    // RSI
    if (rsi14 < 30 && rsi14 > rsi14p) {
        score += 28;
        reasons.push('RSI14 超賣區回升');
    }
    else if (rsi14 < 42 && rsi14 > rsi14p) {
        score += 15;
        reasons.push('RSI 較低位置回升');
    }
    else if (rsi14 >= 40 && rsi14 <= 60 && rsi14 > rsi14p) {
        score += 10;
        reasons.push('RSI 動能健康區間上行');
    }
    // EMA 金叉
    if (e9 > e21 && e9p <= e21p) {
        score += 22;
        reasons.push('EMA9 上穿 EMA21（金叉）');
    }
    else if (e9 > e21 && e9 > e9p) {
        score += 10;
        reasons.push('EMA 多頭排列走強');
    }
    // MACD
    if (dif[i] > dea[i] && dif[p] <= dea[p]) {
        score += 20;
        reasons.push('MACD 金叉');
    }
    else if (dif[i] > dea[i] && hist[i] > hist[p]) {
        score += 10;
        reasons.push('MACD 動量放大');
    }
    // 布林帶
    if (closes[p] < dn[p] && closes[i] > bollDn) {
        score += 15;
        reasons.push('布林下軌強力反彈');
    }
    else if (closes[p] <= mb[p] && closes[i] > bollMb) {
        score += 8;
        reasons.push('突破布林中軌');
    }
    // 成交量
    if (priceUp && volRatio >= 1.8) {
        score += 15;
        reasons.push(`放量上漲（量比 ${volRatio.toFixed(1)}x）`);
    }
    ...
    if (priceNewHigh && rsiDecline)
        smcTopProb += 0.35;
    if (bollUp > 0) {
        const distToUp = (bollUp - closes[i]) / closes[i];
        if (distToUp < 0.005)
            smcTopProb += 0.3;
        else if (distToUp < 0.015)
            smcTopProb += 0.15;
    }
    if (e21 > 0) {
    ...
    if (dif[i] < dea[i] && dif[p] >= dea[p])
        smcTopProb = Math.min(smcTopProb + 0.25, 1.0);
    return {
...
async function getActiveSymbols() {
    try {
        ...
        console.warn('[Scanner] 物化視圖刷新失敗，使用舊數據：', err.message);
    }
    const rows = await query('SELECT symbol FROM active_symbols');
...
async function runScanCycle() {
    ...
    for (const symbol of symbols) {
        try {
            ...
            console.error(`[Scanner] ${symbol} 處理失敗：`, err.message);
        }
    }
    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[Scanner] 掃描完成：${symbols.length} 個標的，` +
...
async function startScanner() {
    ...
    // 首次立即執行
    await runScanCycle().catch(err => console.error('[Scanner] 首次掃描失敗：', err.message));
    // 定時循環
    setInterval(async () => {
        await runScanCycle().catch(err => console.error('[Scanner] 掃描失敗：', err.message));
    }, SCANNER_INTERVAL_MS);
    // 優雅關閉
    const shutdown = () => {
