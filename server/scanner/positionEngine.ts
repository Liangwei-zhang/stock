...
export interface PositionInput {
  ...
  score: number;            // 信號分數 (0-100)
  existingShares?: number;  // 現有持倉股數
  existingAvgCost?: number; // 現有持倉均價
}
...
export interface PositionSuggestion {
  ...
  targetPct: number;        // 目標倉位佔比
  notes: string;
}

/** 根據 score 確定目標倉位比例 */
function getTargetPct(score: number): number {
  if (score >= 90) return 0.20;
  if (score >= 80) return 0.15;
  if (score >= 70) return 0.10;
  return 0.05;
}

/** 計算倉位建議 */
export function calcPosition(input: PositionInput): PositionSuggestion | null {
  const {
    totalCapital, availableCash, currentPrice, score,
    existingShares = 0, existingAvgCost = 0,
  } = input;
  ...
  if (spendable <= 0) return null; // 現金不足
  ...
  if (remainAllowed <= 0) return null; // 已超上限

  targetAmount = Math.min(targetAmount, spendable, remainAllowed);
  if (targetAmount < currentPrice) return null; // 不夠買 1 股
...
function calcNewAvgCost(
  oldShares: number, oldAvgCost: number,
  addShares: number, addPrice: number
): number {
