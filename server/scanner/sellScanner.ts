...
export interface SellSignal {
  symbol: string;
  currentPrice: number;
  smcTopProbability?: number; // 0-1，SMC 頂部信號強度（可選）
}
...
function calcSellAction(
  ...
  stopLoss: number,
  smcTopProbability = 0
): { sellPct: number; reason: string } | null {
 ...
 */
export async function processSellSignal(signal: SellSignal): Promise<void> {
  ...
  const portfolios = await query<PortfolioRow>(
    `SELECT
     ...
     WHERE p.symbol = $1 AND p.notify = true AND u.is_active = true`,
    [symbol]
  );
  ...
  for (const p of portfolios) {
    ...
    const action = calcSellAction(
      shares, avgCost, currentPrice,
      targetProfit, stopLoss, smcTopProbability
    );
    if (!action) continue;
    ...
    const action2 = 'sell' as const;

    // 取 signal_id（用同一個）
    const sigRow = await queryOne<{ id: string }>(
      `INSERT INTO signals (symbol, type, score, price, reasons)
       VALUES ($1, $2::signal_type, $3, $4, $5)
       RETURNING id`,
      [symbol, tradeType, isStopping ? 30 : 75, currentPrice, JSON.stringify([action.reason])]
    );
    if (!sigRow) continue;
...
function buildSellEmailHtml(p: SellEmailParams): string {
