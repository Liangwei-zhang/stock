...
export async function saveEmailCode(
  ...
  await query(
    `INSERT INTO email_codes (email, code, ip, expires_at)
     VALUES (lower($1), $2, $3::inet, $4)`,
...
export async function verifyEmailCode(
  ...
  if (cached && cached === code) {
    ...
    query(
      ...
      [lEmail, code]
    ).catch(err => console.error('[auth] 標記驗證碼失敗：', err.message));
    return true;
  }
...
export async function upsertUser(
  ...
  return transaction(async (client: PoolClient) => {
    ...
    // 創建空資金帳戶（等待引導頁設置）
    await client.query(
      `INSERT INTO user_account (user_id, total_capital, currency)
       VALUES ($1, 0, 'USD') ON CONFLICT DO NOTHING`,
      [rows[0].id]
    );
    return rows[0];
  });
...
export async function createSession(
  ...
  await query(
    `INSERT INTO sessions (token_hash, user_id, device_info, expires_at)
     VALUES ($1, $2, $3, $4)`,
    [tokenHash, userId, JSON.stringify(deviceInfo), expiresAt.toISOString()]
  );
...
export async function verifySession(token: string): Promise<string> {
  ...
  if (!session) throw new Error('TOKEN_REVOKED');
...
export async function isCodeRateLimited(email: string): Promise<boolean> {
