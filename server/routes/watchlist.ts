...
// 查詢 user plan 的最大關注數
async function getMaxWatchlist(userId: string): Promise<number> {
  const row = await queryOne<{ plan: string }>(
    `SELECT plan FROM users WHERE id = $1`,
    [userId]
  );
  if (!row) return 10;
  const limits: Record<string, number> = { free: 10, pro: 50, premium: 200 };
...
router.post('/', validate(addSchema), asyncHandler(async (req, res) => {
  ...
  const row = await queryOne(
    `INSERT INTO user_watchlist (user_id, symbol, notify, min_score)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (user_id, symbol) DO UPDATE SET
       notify = EXCLUDED.notify,
       min_score = EXCLUDED.min_score
     RETURNING *`,
    [userId, symbol, notify, min_score]
  );
...
router.put('/:id', validate(updateSchema), asyncHandler(async (req, res) => {
  ...
  if (body.min_score !== undefined) { params.push(body.min_score); updates.push(`min_score = $${params.length}`); }
  ...
  const row = await queryOne(
    `UPDATE user_watchlist SET ${updates.join(', ')}
