...
import { getWatchlist, upsertWatchlistItem, removeWatchlistItem, pruneOldHistory, } from './storageService';
// ─── 模拟数据生成 ─────────────────────────────────────────────────────────────
const BASE_PRICES = {
...
function genSimulated(symbol, name, basePrice = 100, vol = 0.02, days = 120) {
    const data = [];
    ...
    for (let i = days; i >= 0; i--) {
        ...
        if (dow === 0 || dow === 6)
            continue;
        if (--tDown <= 0) {
            tDir = Math.random() > 0.45 ? 1 : -1;
            tDown = 5 + Math.floor(Math.random() * 10);
        }
...
function simTick(last, vol = 0.02) {
...
class StockService {
    ...
    // ── 后台拉取真实历史，2 路并发 ───────────────────────────────────────────────
    async _refreshAllHistories() {
        const items = Array.from(this.watchlist.values());
        const CONCURRENCY = 2;
        for (let i = 0; i < items.length; i += CONCURRENCY) {
            await Promise.allSettled(items.slice(i, i + CONCURRENCY).map(item => this._fetchRealHistory(item)));
            if (i + CONCURRENCY < items.length)
                await new Promise(r => setTimeout(r, 400));
        }
    }
    async _fetchRealHistory(item) {
        try {
    ...
    async _updateSymbol(symbol) {
        const item = this.watchlist.get(symbol);
        if (!item)
            return;
        const cached = await marketDB.getCachedQuote(symbol);
        if (cached) {
    ...
    // ── 添加 / 移除 ─────────────────────────────────────────────────────────────
    async addSymbol(item) {
        if (this.watchlist.has(item.symbol))
            return;
        this.watchlist.set(item.symbol, item);
        this.meta.set(item.symbol, { source: 'simulated', lastUpdated: 0 });
        const base = BASE_PRICES[item.symbol] ?? 100;
    ...
    getStockHistory(symbol) { return this.data.get(symbol) ?? []; }
    getKLineData(symbol) {
    ...
    getAvailableStocks() { return Array.from(this.watchlist.keys()); }
    hasSymbol(symbol) { return this.watchlist.has(symbol); }
