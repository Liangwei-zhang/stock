...
// ─── 通用事务工具 ─────────────────────────────────────────────────────────────
function withTx(mode, stores, fn) {
...
export async function getWatchlist() {
    try {
        return await withTx('readonly', 'watchlist', tx => tx.objectStore('watchlist').getAll());
    }
    catch {
        return [];
    }
...
export async function upsertWatchlistItem(item) {
    try {
        await withTx('readwrite', 'watchlist', tx => tx.objectStore('watchlist').put(item));
    }
    catch (e) {
        console.warn('upsertWatchlistItem failed', e);
    }
}
export async function removeWatchlistItem(symbol) {
    try {
        await withTx('readwrite', 'watchlist', tx => tx.objectStore('watchlist').delete(symbol));
    }
    catch (e) {
        console.warn('removeWatchlistItem failed', e);
    }
}
// ─── History ──────────────────────────────────────────────────────────────────
function toRecord(d) {
...
export async function getHistory(symbol, displayName = symbol) {
    try {
...
export async function pruneOldHistory(symbol) {
    try {
        const db = await openDB();
        const cutoff = Date.now() - SIX_MONTHS_MS;
        await new Promise((resolve, reject) => {
        ...
        console.warn('pruneOldHistory failed', e);
    }
}
/** 删除某 symbol 的所有 history */
export async function clearSymbolHistory(symbol) {
    try {
        const db = await openDB();
        await new Promise((resolve, reject) => {
            const tx = db.transaction('history', 'readwrite');
        ...
        console.warn('clearSymbolHistory failed', e);
    }
}
/** 保存 POI 列表 */
export async function savePOIs(symbol, pois) {
    try {
        const db = await openDB();
        ...
        console.warn('savePOIs failed', e);
    }
}
/** 載入 POI 列表 */
export async function loadPOIs(symbol) {
    try {
        const db = await openDB();
        return new Promise((resolve, reject) => {
...
/** 清除 POI */
export async function clearPOIs(symbol) {
    try {
        const db = await openDB();
        const tx = db.transaction('pois', 'readwrite');
        const store = tx.objectStore('pois');
        ...
        console.warn('clearPOIs failed', e);
    }
}
